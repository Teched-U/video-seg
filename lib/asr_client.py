    ...
    def run(self):
        try:
            self.ws = yield websocket_connect(
                self.url, on_message_callback=self.received_message
            )
            if self.send_adaptation_state_filename is not None:
                print(
                    "Sending adaptation state from "
            ...
            with self.audiofile as audiostream:
                while True:
                    block = yield from self.ioloop.run_in_executor(
                        self.executor, audiostream.read, int(self.byterate / 4)
                    )
                    if block == b"":
                        break
...
def main():
    ...
    ws = MyClient(
        args.audiofile,
        args.uri + "?%s" % (urllib.parse.urlencode([("content-type", content_type)])),
        byterate=args.rate,
