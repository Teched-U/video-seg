...
def contours_text(orig, contours, frame_index):
    ...
    for cnt in contours: 
        ...
        if (len(text) > 0):
            ...
            print(pytesseract.image_to_data(cropped, config = config))
            n_boxes = len(d['text'])
            sum = 0
            count = 0
...
def get_Canny(pathIn):
    ...
    while success:
        ...
        plt.imshow(image)
        plt.show()
        cv2.imwrite("/Users/yizhizhang/Downloads/test_frames4/" + str(count) + ".jpg", image)  # save frame as JPEG file
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        canny = cv2.Canny(gray,50,150)
        frames.append(canny)
        imgs.append(image)
        count += 1
        vid_cap.set(cv2.CAP_PROP_POS_MSEC, 3 * 1000 * count)
        success, image = vid_cap.read()
...
def main():
    pathin = "/Users/yizhizhang/Downloads/chinese_test.mp4"
    ...
    for differential in differentials:
        ...
        arr = arr[:,0:int(0.7 * differential.shape[0])]
        arr = arr != 255
        # CC Analysis
    ...
    #     print(i)
        cv2.imwrite("/Users/yizhizhang/Downloads/test_frames4/key" + str(i) + ".jpg", key_frames[i])
    # #     plt.show()
    ...
    #     #change to bool image
    #     arr = np.asarray(differential)
    #     arr = arr[0:int(0.3 * differential.shape[1]),:]
    #     arr = arr != 255
    #     # CC Analysis
    #     result = connected_component_labelling(arr, 4)
    ...
    # # index2 = np.array([0, 1,  3,  4,  6,  8, 11, 13, 14, 15, 16])
    # key_frames_index = np.array([index[i] for i in index2])
    
    # final_key_frames = [imgs[i] for i in key_frames_index]
    
    # for i in range(0,len(key_frames_index)):
    # #     print(i)
    #     cv2.imwrite("/Users/yizhizhang/Downloads/test_frames4/Finalkey" + str(i) + ".jpg", final_key_frames[i])
...
if __name__ == "__main__":
    ...
    #     CCs.append(np.max(result))
    #     print(np.max(result))
    # image = cv2.imread("/Users/yizhizhang/Downloads/test_frames2/key3.jpg")
    # # #image = cv2.imread("/Users/yizhizhang/Desktop/andrew.png")
    # gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    # cnts = detect_paragraph(gray)
    # contours_text(gray, cnts,0)